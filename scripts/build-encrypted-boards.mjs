...
if (hasPrivateBoards && !masterKey) throw new Error('BOARD_CONTENT_MASTER_KEY가 필요합니다.');
const passwords = hasPrivateBoards ? JSON.parse(decrypt(JSON.parse(fs.readFileSync(path.join(source, 'passwords.enc.json'), 'utf8')), masterKey)) : {};
const publicBoards = [];
