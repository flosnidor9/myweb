...
if (!fs.existsSync(source)) { writeManifest([]); console.log('비공개 게시판 소스가 없어 빈 게시판 목록을 생성했습니다.'); process.exit(0); }
if (!masterKey) throw new Error('BOARD_CONTENT_MASTER_KEY가 필요합니다.');
...
if (boards.length === 0) { writeManifest([]); console.log('등록된 게시판이 없어 빈 게시판 목록을 생성했습니다.'); process.exit(0); }
const encryptedPasswords = JSON.parse(fs.readFileSync(path.join(source, 'passwords.enc.json'), 'utf8'));
const passwords = JSON.parse(decrypt(encryptedPasswords, masterKey));
const publicBoards = [];
...
writeManifest(publicBoards); console.log(`${publicBoards.length}개 게시판을 암호화했습니다.`);
